...
export function useGeolocation(defaultLoc) {
  ...
  // Pull the admin-configured fallback (set in /admin/settings) so the "before consent /
  // permission denied" location reflects what the org set, not just a hardcoded Jakarta.
  // Only applied if geolocation hasn't already resolved by the time this responds.
  useEffect(() => {
    let cancelled = false;
    fetch("/api/settings/default-location", { signal: AbortSignal.timeout(5000) })
      .then((res) => res.json())
      .then((data) => {
        if (cancelled || !data.ok) return;
        setLoc((current) => (current === defaultLoc ? { lat: data.lat, lon: data.lon, label: data.label } : current));
      })
      .catch(() => {});
    return () => {
      cancelled = true;
