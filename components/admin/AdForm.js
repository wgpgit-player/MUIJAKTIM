...
export default function AdForm({ action, initial }) {
  const [active, setActive] = useState(initial?.active ?? true);

  return (
    <form action={action} className="flex flex-col gap-5 max-w-xl">
      <div>
        <label className="block text-[12.5px] font-bold text-ink mb-1.5">Gambar Iklan</label>
        {initial?.imageUrl && (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={initial.imageUrl}
            alt=""
            className="w-full max-w-sm h-24 object-cover rounded-xl border border-line mb-2 bg-cream"
          />
        )}
        <input
          type="file"
        ...
        />
        <p className="text-[11.5px] text-ink-soft mt-1">Rasio disarankan 970×90 (leaderboard) untuk slot beranda.</p>
        <input type="hidden" name="currentImageUrl" value={initial?.imageUrl ?? ""} />
      </div>

      <div>
        <label className="block text-[12.5px] font-bold text-ink mb-1.5">Judul / Nama Sponsor</label>
        <input
          name="title"
          required
      ...
      <div>
        <label className="block text-[12.5px] font-bold text-ink mb-1.5">Slot Penayangan</label>
        <input
          name="placement"
          required
          defaultValue={initial?.placement ?? "homepage_leaderboard"}
          className="w-full border border-line rounded-xl px-4 py-2.5 text-[13.5px] outline-none focus:border-emerald"
        />
        <p className="text-[11.5px] text-ink-soft mt-1">
          Kode slot tempat iklan tayang. Default beranda: <code>homepage_leaderboard</code>.
        </p>
      </div>
      <div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-[12.5px] font-bold text-ink mb-1.5">Mulai Tayang (opsional)</label>
          <input
            type="date"
            name="startAt"
            defaultValue={toDateInputValue(initial?.startAt)}
            className="w-full border border-line rounded-xl px-4 py-2.5 text-[13.5px] outline-none focus:border-emerald"
          />
        </div>
        <div>
          <label className="block text-[12.5px] font-bold text-ink mb-1.5">Selesai Tayang (opsional)</label>
          <input
            type="date"
            name="endAt"
            defaultValue={toDateInputValue(initial?.endAt)}
            className="w-full border border-line rounded-xl px-4 py-2.5 text-[13.5px] outline-none focus:border-emerald"
          />
        </div>
      </div>
      <p className="text-[11.5px] text-ink-soft -mt-3">Kosongkan agar tayang terus tanpa batas waktu.</p>

      <div className="grid grid-cols-2 gap-4 items-end">
        <div>
          <label className="block text-[12.5px] font-bold text-ink mb-1.5">Urutan</label>
          <input
            type="number"
            name="sortOrder"
